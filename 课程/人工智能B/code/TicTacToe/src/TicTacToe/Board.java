...
public class Board {
    ...
    public enum State {
        /**
         * chess state, there are no pieces on the board
         */
        Blank,
        /**
         * chess state, X chess piece is here
         */
        X,
        /**
         * chess state, O chess piece is here
         */
        O
    }
    ...
    void reset() {
        moveCount = 0;
        gameOver = false;
        playersTurn = Math.random() >= 0.5 ? State.X : State.O;
        winner = State.Blank;
        initialize();
    }
     ...
     * @param y the y coordinate of the location
     */
    public void checkIfWin(int x, int y) {
        checkRow(y);
        checkColumn(x);
