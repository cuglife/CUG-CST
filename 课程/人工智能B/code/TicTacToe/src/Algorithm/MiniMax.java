...
class MiniMax {
    ...
    private static double maxPlayer;
     ...
     * @param maxPlayer the maximum depth
     */
    static void run(Board.State player, Board board, double maxPlayer) {
        if (maxPlayer < 1) {
            throw new IllegalArgumentException("Maximum depth must be greater than 0.");
        }

        MiniMax.maxPlayer = maxPlayer;
    ...
    private static int miniMax(Board.State player, Board board, int currentPly) {
        if (currentPly++ == maxPlayer || board.isGameOver()) {
